...
def count_dup(l):
    ...
    for a in l:
        ...
        d[a] += 1
    return d
# print(count_dup([3, 4, 4, 5,  4, 3, 2, 1, 1]))
...
def insert_sort(d):
    ...
    new_list = []
    for a,b in d.items():
        new_list = insert_order(new_list, Elem(a, b) )
            
    return new_list 


def top_k_duplicated_elem(l, k):
    d = count_dup(l)
    sorted_list = insert_sort(d)
    last_k = sorted_list[-k:]
...
print(top_k_duplicated_elem(list_1, 2))
